...
var isIE = navigator.appName === 'Microsoft Internet Explorer' &&
           navigator.userAgent.indexOf('Opera') < 1 ? 1 : 0;
...
function go(step) {
  ...
  //hallvord
    ...
    if (ce.getAttribute("onslidehide")) {
      var hidefun = new Function(ce.getAttribute("onslidehide"));
      hidefun.apply(window);
    }

		ce.style.visibility = 'hidden'; 
		ne.style.visibility = 'visible';

    if (ne.getAttribute("onslideshow")) {
      var showfun = new Function(ne.getAttribute("onslideshow"));
      showfun.apply(window);
    }
	} // /hallvord
...
// Loads a note into the note window
	...
	var jl = document.getElementById('jumplist');
	var slideTitle = jl.options[jl.selectedIndex].text.replace(/^\d+\s+:\s+/, '').replace(/[<>]/,'') + ((jl.selectedIndex) ? ' (' + jl.selectedIndex + '/' + (smax - 1) + ')' : '');
	if (incrementals[snum].length > 0) {
//		alert('howdy');
...
function updateSlideview() {
	if (s5NotesWindow && !s5NotesWindow.closed && s5NotesWindow.document) {
                ...
                var slideview = s5NotesWindow.document.getElementById('slideview_canvas');
                var ctx = slideview.getContext('2d');
                ctx.save();
                var scale = slideview.width / document.width;
                ctx.scale(scale, scale);
                ctx.drawWindow(window, 0, 0, document.width, slideview.height / scale, "#ffffff");
                ctx.restore();
        }
}
