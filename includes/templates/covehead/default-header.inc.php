...
array_unshift($css, 'covehead/global');
  ...
  <body id="<?= $body_id ?>" class="<?= $body_class ?>" role="document" itemscope itemtype="http://schema.org/<?= $page_type ?>" itemref="copyright">
    <div id="wrapper">
      <header id="pageheader" role="banner" itemscope itemtype="http://schema.org/WPHeader">
        <div id="globalheader">
        <?php if (strpos($file_path, '/firefox/') === 0): ?>
          <nav role="navigation" aria-label="Firefox 製品情報ナビゲーション" itemscope itemtype="http://schema.org/SiteNavigationElement" id="nav-firefox">
            <h1 role="presentation"><a href="https://www.mozilla.org/ja/firefox/">Firefox</a></h1>
        <?php elseif (strpos($file_path, '/thunderbird/') === 0): ?>
          <nav role="navigation" aria-label="Thunderbird 製品情報ナビゲーション" itemscope itemtype="http://schema.org/SiteNavigationElement" id="nav-thunderbird">
